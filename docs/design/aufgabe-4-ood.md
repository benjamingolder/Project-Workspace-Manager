...
| `persistence-schicht.puml` / `.png` | UML-Klassendiagramm Persistence-Schicht |
| `business-schicht.puml` / `.png` | UML-Klassendiagramm Business-Schicht |
| `integriert-klassendiagramm.puml` / `.png` | UML-Klassendiagramm integriert (alle 3 Schichten) |
| `integriert-komponentendiagramm.puml` / `.png` | UML-Komponentendiagramm integriert |
| `sequenzdiagramm-fa01.puml` / `.png` | UML-Sequenzdiagramm Normalablauf FA01 |

> Die `.puml`-Dateien sind die Quelldateien, die `.png`-Dateien sind die daraus generierten Bilder. Für die Abgabe können die Diagramme zusätzlich in Visual Paradigm nachgebaut und exportiert werden.
